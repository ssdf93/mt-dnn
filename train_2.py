...
SAMPLERS={
    "random" : MultiTaskBatchSampler,
    "order" : OrderedBatchSampler,
    "acl" : ACLSampler
}
...
def train_config(parser):
    ...
    parser.add_argument('--sampler', default='random',
                        help='supported optimizer: random, order, epison')
...
def main():
    ...
    n_tasks = len(tasks)
    controller = ACLController(n_task=n_tasks, batch_size=None, phi=args.phi)
    ...
    if args.sampler == "mab":
        model.sampler = multi_task_batch_sampler
    ...
    for epoch in range(0, args.epochs):
        ...
        start = datetime.now()
        total_len = len(multi_task_train_data)
